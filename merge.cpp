...
void merge(int * arr, int start, int end)
{
    ...
    // The arr actally has been sorted till start to mid and also from mid to end but now we need to merge(combine these 2)
    ...
    int main_arr_index; // So that we can keep track of the mian array we are currently transversing
    ...
    for(i=0;i<len1;i++)
    {
        arr1[i] = arr[main_arr_index];  //This is basically copying all of the first half of the sorted arrays
        main_arr_index++;
    }
    ...
    // We only need to increase the index of the element which is less and is actually getting placed in the main arr 

    while(index_1<len1 && index_2<len2)
    {
        if(arr1[index_1]<arr2[index_2])
        {
            // This means the elements in arr1 are lesser, so since we are sorting by increasing order, this will be placed in the arr
    ...
    // Now this is till one of the arrays is completely iterated
...
void mergeSort(int *arr, int start, int end)
{
    ...
    if(start>=end) //This implies there is only one 1 element and now we can not split it further 
    {
        return;
    }
